...
namespace JoySlots.ViewModel
{
    public class SlotsGameViewModel : BaseViewModel
    {
        public ImageSource _volumeState;
        public ImageSource VolumeState
        {
            get => _volumeState;
            set
            {
                if (value != _volumeState)
                {
                    _volumeState = value;
                    OnPropertyChanged();
                }
        ...
        {
            _volumeState = (App.Current.Resources["img_VolumeMax"] as ImageSource)!;
        ...
        public async Task<List<WinningLine>> CheckWinningLines(Grid ReelsGrid)
        {
            ...
            if (WinningLines.Count > 0)
            {
                ...
                Logger.LogInfo("SlotsGameViewModel/CheckWinningLines", $"Winning lines\nCount={WinningLines.Count}" +
                    $"\nLines={lineWon}\nAmountWon={AmountWon}");
            }
